...
* <a href="#clust">Cluster analysis  and variable selection with unknown number of clusters</a>.
* <a href="#discrim">Discriminant analysis with variable selection</a>.

All the experiments are used with SelvarMix 1.2
<a id="intro"></a>

### Introduction
*SelvarMix* carries out a regularization approach of variable selection is considered 
in the model-based clustering and classification frameworks. First, the
...
in the R package SelvarMix, provides a much faster variable selection procedure 
than [@Maugis2009b;@Maugis2011] and allows us to study high-dimensional
datasets.
...
The cluster analysis is performed with unknown number of clusters by doing a variable selection, 
then a model summary is given. 
...
<a id="clust"></a>
**Clustering with variable selection and number of clusters selection**
```{r, comment=""}
# Cluster analysis with variable selection with parallel computing (8 cores) 
# The last two input arguments are optional
require(SelvarMix)
junk <- SelvarClustLasso(x=x, nbcluster=3:5, models=mixmodGaussianModel(family = "spherical"), nbcores=8)
```
...
junk <- SelvarLearnLasso(x=xl, z=zl, models=mixmodGaussianModel(family = "spherical"), xtest=xt, ztest=zt,nbcores=8)
```
**Model Summary**
